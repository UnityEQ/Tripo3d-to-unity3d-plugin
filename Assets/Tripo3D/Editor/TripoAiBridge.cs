    ...
    internal static class TripoAiBridge
    {
        ...
        static string BuildPrompt(string glb, string slug, string outDir)
        {
            ...
            return
                ...
                "2. Fit anatomical landmarks and build the exact 52-bone export skeleton. Names/parents are the v1 tokens (`Hips`, `Clavicle.L`, `UpperArm.R`, `Thumb1.L`, …) — PascalCase + Blender `.L`/`.R`. Do **not** Mixamo-rename or Unreal-rename. Armature object `RIG-<slug>`, mesh `GEO-<slug>`, vertex groups = bone names, IK controls `CTRL_*`. Optional IK, FK default. See the standard's Unity Humanoid map.\n" +
                "3. Skin / bone weights: follow **§4 of the Modular standard**. First bind (`run.py` `heat_bind`) already isolates opposite-limb/finger leak, spreads hinge bands, and elf-prunes to four influences — do not skip `refine_weights` or replace it with a whole-mesh Smooth. Then pose elbows/knees/shoulders/crotch at ~90° and finish §4b joint bands. Do not add twist bones, Preserve Volume, or Corrective Smooth. A bind that still spikes at 90° is not done.\n" +
                ...
                "   - `Idle` — looping, 60 frames (2.0s). Author this first. Its frame-1 stand is the shared rest pose. Subtle breathing, weight shift, slight head/arm motion. In-place. First and last pose must match. Arms hang relaxed at the sides (soft elbow bend, hands near the hips), not held out.\n" +
                "   - `Run` — **Idle → two-step jog-run → Idle**, 24–30 frames. Opposite arm/leg, hip bounce, full-body. In-place (no forward travel) so Unity can add root motion. Frame 1 and the last frame **are** the Idle stand (copy Idle frame 1). Do not start or end mid-stride. Arms pump close to the torso like a real run, not a wide A-pose.\n" +
                "   - `Jump` — **Idle → crouch → takeoff → hang → land → Idle**, 30–45 frames, **not** looping. Vertical motion on Hips/Root only. Frame 1 and the last frame **are** the Idle stand, not idle-like and not a ready squat. Arms stay near the body (counterbalance), not stuck out to the sides.\n" +
                "   - `SwordSlash` — **Idle → slash → Idle**, 24–36 frames, **not** looping. Use **Hand.R** as if gripping a one-handed sword: wind-up over the **right-rear / high-right** (not through the head or back), then cut in an arc **in front of** the chest, follow-through to the left-front, recover to Idle. Frame 1 and the last frame **are** the Idle stand (grip starts after frame 1; released by the last frame). Do **not** create a sword mesh unless one already exists. Left arm may counterbalance **outside** the torso. Keep the right elbow in a natural slash path, not flared wide.\n" +
                "   **Idle bookends (required for Unity transitions).** Copy Idle frame 1 onto frame 1 and the last frame of Run, Jump, and SwordSlash. Same pose and near-zero velocity. Unity crossfades Idle ↔ every other clip; if a clip starts or ends mid-action, the blend pops.\n" +
